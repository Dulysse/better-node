...
function _replaceSepWords(str, sep) {
	...
	if (str[index + 1]) {
		return _replaceSepWords(
			`${str.slice(0, index)}${str[index + 1].toUpperCase()}${str.slice(
				index + 2
			)}`,
			sep
		);
...
try {
	Object.defineProperty(global.String.prototype, "capitalized", {
		value: function (sep, first = true) {
			if (sep && !separators.includes(sep)) {
				throw new Error(
					`InvalidSeparator: Capitalize separator must be: "${separators.join(
						'" or "'
					)}".`
				);
			}
